...
import { heartbeatDataModel } from './heartbeatDataModel';

heartbeatData = heartbeatDataModel;

let lastHeartRates = [];

function calcSmoothedPrediction(lastHeartRates) {

    let sum = lastHeartRates.reduce((previous, current) => current += previous);
    let avg = sum / lastHeartRates.length;

    heartbeatData.heartRate = avg;  
}
...
// https://github.com/prouast/heartbeat-js/blob/master/LICENSE
...
export class Heartbeat {
  ...
  detectFace(gray) {
    ...
    if (faces.size() > 0) {
      ...
      toastr.success('Make sure you are in frame', 'No face found yet');
      this.invalidateFace();
    }
    faces.delete();
  }
  ...
  rppg() {
    ...
    if (this.signal.length >= this.targetFps * this.windowSize) {
      ...
      if (!signal.empty()) {
        ...
        // Push for final time window average
        let maxlastHeartRates = 10;
        if (lastHeartRates.length > maxlastHeartRates) {
          //do a removal based on last breath states
          lastHeartRates.shift();
          lastHeartRates.push(bpm);
          //console.log(lastHeartRates)

          calcSmoothedPrediction(lastHeartRates);
      ...
      signal.delete();
        ...
        toastr.success('Please wait', 'Collecting data...');
    }
  }
  ...
  drawTime(signal) {
    ...
    for (var i = 1; i < signal.rows; i++) {
      ...
      cv.line(this.overlayMask, start, end, [51, 255, 255, 0], 2, cv.LINE_4, 0);
      start = end;
    }
  }
  ...
  drawBPM(bpm) {
    cv.putText(this.overlayMask, bpm.toFixed(0).toString(),
      ...
      cv.FONT_HERSHEY_PLAIN, 1.5, [51, 255, 255, 0], 2);
  }
  // Clean up resources
  stop() {
